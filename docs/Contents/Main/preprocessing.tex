...
\begin{itemize}
    \item \✓ No NaN (missing) values in any feature column
    \item \✓ No infinite ($\pm \infty$) values in data
    \item \✓ All feature values in expected normalized range [$-3, +3$]
    \item \✓ No duplicate rows in any partition
    \item \✓ Target labels contain only 0 and 1 (no other values)
    \item \✓ No temporal leakage (train $<$ validation $<$ test chronologically)
    \item \✓ Feature statistics computed from train only
\end{itemize}
